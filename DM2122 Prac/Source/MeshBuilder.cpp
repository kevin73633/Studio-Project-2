...
Mesh* MeshBuilder::GenerateSphere(const std::string& meshName, Color color, unsigned numStack, unsigned numSlice, float radius)
{
	...
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mesh->indexBuffer);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, index_buffer_data.size() * sizeof(GLuint), &index_buffer_data[0], GL_STATIC_DRAW);
	mesh = InitializeCollisionPoints(mesh, vertex_buffer_data);
	return mesh;
}
...
Mesh* MeshBuilder::GenerateText(const std::string& meshname, unsigned numRow, unsigned numCol)
{
	...
	for (unsigned j = 0; j < numRow; ++j)
	{
		...
		{
			v.pos.Set(0, 1, 0);
			//v.texCoord.Set(i / numRow, (1 + j) / numCol);
...
Mesh* MeshBuilder::InitializeCollisionPoints(Mesh* mesh, std::vector<Vertex> vertex_buffer_data)
{
	float xmin;
	float xmax;
	float ymin;
...
Mesh* MeshBuilder::GenerateCollisonBox(const std::string& meshname, Vector3 p1, Vector3 p2, Vector3 p3, Vector3 p4, Vector3 p5, Vector3 p6, Vector3 p7, Vector3 p8)
{
	// An array of 3 vectors which represents 3 vertices
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	...
	vertex_buffer_data.push_back(v);
	v.color = color;
	v.pos.Set(p3.x, p3.y, p3.z);
	vertex_buffer_data.push_back(v);
	v.color = color;
	v.pos.Set(p4.x, p4.y, p4.z);
	vertex_buffer_data.push_back(v);
	v.color = color;
	v.pos.Set(p5.x, p5.y, p5.z);
	vertex_buffer_data.push_back(v);
	v.color = color;
	v.pos.Set(p6.x, p6.y, p6.z);
	vertex_buffer_data.push_back(v);
	v.color = color;
	v.pos.Set(p7.x, p7.y, p7.z);
	vertex_buffer_data.push_back(v);
	v.color = color;
	v.pos.Set(p8.x, p8.y, p8.z);
	vertex_buffer_data.push_back(v);
